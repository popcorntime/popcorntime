...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'A checkbox component built with Radix UI primitives, featuring proper accessibility and styling.',
      },
    },
  },
  tags: ['autodocs'],
...
export const Default: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("should render unchecked checkbox", async () => {
    ...
    await step("should be unclickable", async () => {
      const checkbox = await canvas.findByRole("checkbox");
      await userEvent.click(checkbox);
      expect(checkbox).not.toBeChecked();
    });
...
export const Indeterminate: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Checkbox in indeterminate state, useful for parent checkboxes in hierarchical lists',
      },
    },
  },
}
    ...
    return (
      <div className="space-y-2">
        {items.map((item, index) => (
          <div key={item.id} className={`flex items-center space-x-2 ${index === 0 ? 'border-b pb-2 mb-2' : ''}`}>
